...
from src.dispute_resolution import DisputeWorkflow, ModelAuditClient, OlistRepository, RuntimeConfig
from src.dispute_resolution.workflow import CaseError, POLICY_VERSION, TraceWriter
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resolve Olist dispute cases using EC_POLICY_V1.")
...
def write_metadata(path: Path, config: RuntimeConfig, repository: OlistRepository, case_count: int) -> None:
    metadata = {
...
def run() -> int:
    ...
    workflow = DisputeWorkflow(repository, trace, audit_client, max_workers=config.max_workers)
    failures: list[str] = []
    try:
        for input_path in files:
            try:
                ...
                write_json(args.output_dir / input_path.name, output)
    ...
    write_metadata(args.metadata_path, config, repository, len(files) - len(failures))
    if failures:
